...
class TestWechat:
     ...
     def test_add_member(self):
         ...
         #脚本里断言，判断进入"通讯录界面"
         if not self.contactPage.is_in_contact_page():
             raise AssertionError("进入'通讯录界面'失败！")
         ...
         time.sleep(3)
         # 脚本里断言，判断进入"添加成员"界面
         if not self.addMemberPage.is_in_add_member_page():
             raise AssertionError("进入'添加成员'界面失败！")
